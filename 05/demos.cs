...
namespace OptionPricerEngine
{
    class Program
    {
        ...
        [DllImport("simulation_engine.so", CallingConvention = CallingConvention.Cdecl)]
        private static extern float CalculateCallValueGpu(OptionData optionData, int paths);
        ...
        static void Main(string[] args)
        {
            ...
            var results = allOptionData.Select(x => new SimulationResult 
                                                    { 
                                                        OptionId = x.Id, 
                                                        CallValue = CalculateCallValueGpu(x, NumberOfPaths) 
                                                    });
